...
class UserModel extends Model
{
    ...
    public static function saveInfo($openid,$mobile)
    {
        ...
        if (!empty($info)){
            //判断是否绑定过手机号
            try{
                $info=Db::name('user_weixin')->where("user_id=".$info['user_id'])->find();
                //如果绑定过微信就覆盖以前的微信信息
                $data['wx_openid']=$openid;
                Db::where("user_id=".$info['user_id'])->update($data);

                //获取新微信信息
                $weixin_data=UserWeixinModel::where("third_id='".$openid."'")->find();
                $weixin_data=$weixin_data['third_data'];
                ...
                UserModel::where("mobile='".$mobile."'")->update($userData);
